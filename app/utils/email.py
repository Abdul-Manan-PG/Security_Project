...
def send_2fa_code_email(user_email, code):
    ...
    email_logger.debug(f"Attempting to send 2FA code to: {user_email}")
    
    email_body = f"""
    <h2>Your Verification Code</h2>
    <p>Hello,</p>
    <p>Your verification code is:</p>
    <h1 style="font-size: 32px; letter-spacing: 8px; text-align: center; background-color: #f3f4f6; padding: 20px; border-radius: 8px;">{code}</h1>
    <p>This code will expire in 10 minutes.</p>
    <p>If you did not request this code, please ignore this email or contact support if you're concerned about your account security.</p>
    """
    
    msg = Message(
        subject='Your Verification Code - SOC Dashboard',
    ...
    try:
        email_logger.info(f"📧 SENDING 2FA CODE EMAIL | Recipient: {user_email}")
        mail.send(msg)
        email_logger.info(f"✅ 2FA CODE EMAIL SENT SUCCESSFULLY | Recipient: {user_email}")
        return True
    except Exception as e:
        email_logger.error(f"❌ FAILED TO SEND 2FA CODE EMAIL | Recipient: {user_email} | Error: {str(e)}", exc_info=True)
        return False
